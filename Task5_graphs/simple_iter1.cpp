...
namespace simple_iter{
    ...
    double target_discrepancy=0.1, unsigned frequency_checking=5u, unsigned max_iteration=100u){
        std::fstream out1("result5.txt", std::ios_base::app);
        auto res = vector(x_0);
    ...
    double target_discrepancy=0.1, unsigned frequency_checking=5u, unsigned max_iteration=100u){
        auto res = vector(x_0);
        std::fstream out2("result6.txt", std::ios_base::app);
        for (auto i = 0u; i < max_iteration; i++){
            {
                Timer t(out2);
    ...
    double target_discrepancy=0.1, unsigned frequency_checking=5u, unsigned max_iteration=100u){
        auto res = vector(x_0);
        std::fstream out3("result7.txt", std::ios_base::app);
        for (auto i = 0u; i < max_iteration; i++){
            {
                Timer t(out3);
                res = res - tau * (A * res - b);
    ...
    unsigned frequency_checking=3u, unsigned max_iteration=100u, unsigned quantity_roots = 3u){
        auto res = vector(x_0);
        std::fstream out4("result8.txt", std::ios_base::app);
        {
            Timer t(out4);
